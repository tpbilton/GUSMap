---
title: "Introduction to GUSMap (back-cross analysis)"
...
b) The recombination fraction is substantially different in the two parents.
...
In the following sections, we shall discuss how each of these steps are performed in GUSMap for a BC object.

### Constructing a back-cross population

Construction of a 'back-cross' population object is achieved using the `makeBC` function.
```{r makeBC, eval=T}
...
The `makeBC` function takes an `RA` object and pedigree file, creates a back-cross population and performs filtering on the data. 
