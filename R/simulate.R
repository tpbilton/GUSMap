...
#' interference. The processed for the simulation is as follows: \itemize{
#' \item Inhertiance vectors are simulated according to the true recombination
#' fractions, assuming no interference. \item The inheritance vectors are
#' converted to genotypes for the specified OPGP vector (which is simulated
#' based on the \code{config} vector). \item Simulated read depths are
#' generated by simulating realizations from the read depth distribution. The
#' alleles of the true genotype are then sampled with equal probability and
#' replacement until a sample size corresponding to the read depth is obtained,
#' where a sequencing error (e.g. an reference allele called as alternate and
#' vice versa) is simulated to occur with probability \code{epsilon}. The mean
#' read depth for each loci is assumed to be equal. }
#' 
...
#' of the files (if \code{filename="sim"}) is as follows: \itemize{ \item
#' Simulation parameters: sim[No]_info.txt \item GUSMap Genotypes:
#' sim[No]_genon_SEQ.txt \item GUSMap Read depths: sim[No]_depth_Ref_SEQ.txt
#' \item GUSMap Read depths: sim[No]_depth_Alt_SEQ.txt \item OneMap format:
#' sim[No]_OneMap.txt \item LepMap2 format: sim[No]_LepMap.txt \item CriMap
#' format: chr1_sim[No].gen \item JoinMap format: sim[No]_JoinMap.loc } where
#' "No" is the number of the data set simulated (from 1 up to \code{NoDS}).
#' 
#' If none of the methods in \code{formats} argument is specified to be TRUE,
#' then a list containing the following elements will be returned: \itemize{
#' \item genon: Matrix of genotypes for the simulated sequencing data. \item
#' depth_Ref: Matrix of the allele counts for the reference allele. \item
#' depth_Alt: Matrix of the allele counts for the alternate allele. \item
#' trueGeno: Matrix of the true genotypes calls simulated. \item rVec_f: Vector
#' of adjacent paternal recombination fractions. \item rVec_m: Vector of
#' adjacent maternal recombination fractions. \item nInd: Number of
#' individuals. \item nSnps: Number of SNPs. \item config: Vector of
#' segregation types used in the simulation. \item OPGP: Vector of OPGPs used
#' in the simulation. \item meanDepth: Mean of the distribution used to
#' simulate the read depths. \item rd_dist: Distribution from the which the
#' read depths where simulated. \item epsilon: Numeric value of the sequencing
#' error rate used in the simulation. }
