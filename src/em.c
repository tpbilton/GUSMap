...
double computeProb_multierr(int nInd, int nSnps, double pAA[nInd][nSnps], double pBB[nInd][nSnps], double bin_coef[nInd][nSnps],
  ...
  for(ind = 0; ind < nInd; ind++){
    ...
    for(snp = 0; snp < nSnps; snp++){
      ...
      if( (pref[indx] + palt[indx]) == 0){
        ...
        pAA[ind][snp] = bin_coef[ind][snp] * powl(1 - epsilon[snp], pref[indx]) * powl(epsilon[snp], palt[indx]);
        pBB[ind][snp] = bin_coef[ind][snp] * powl(epsilon[snp], pref[indx]) * powl(1 - epsilon[snp], palt[indx]);
      }
    }
  }
  return -1;
...
SEXP EM_HMM_multierr(SEXP r, SEXP ep, SEXP ref, SEXP alt, SEXP OPGP, SEXP noFam, SEXP nInd, SEXP nSnps,
  ...
  // Copy values of R input into C objects
  nSnps_c = INTEGER(nSnps)[0];
  double r_c[(nSnps_c-1)*2], ep_c[nSnps_c], *pep;
  for(snp = 0; snp < nSnps_c - 1; snp++){
    for(parent = 0; parent < 2; parent++){
      r_c[snp+parent*(nSnps_c-1)] = REAL(r)[snp+parent*(nSnps_c-1)];
  ...
  while( (iter < 2) || ((iter < nIter) & ((llval - prellval) > delta))){
    ...
    // unpdate the probabilites for pAA and pBB given the parameter values and data.
    computeProb_multierr(nTotal, nSnps_c, pAA, pBB, bin_coef, pep, pref, palt, nThreads_c);
    ...
    // Error parameter:
    if(seqError_c){
      #pragma omp parallel for private(fam, ind, a, b, s1, sumA, sumB) num_threads(nThreads_c)
      for(snp = 0; snp < nSnps_c; snp++){
        sumA = 0;
        sumB = 0;
        for(fam = 0; fam < noFam_c; fam++){
